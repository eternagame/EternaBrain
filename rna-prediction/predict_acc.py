...
for i in range(len(test_real_X)):
    if np.count_nonzero(np.array(test_real_X[i][:400])) >= 351 and np.count_nonzero(np.array(test_real_X[i][:400])) <= 400:
...
for i in range(len(specs_X)):
    ...
    if np.argmax(specs_y[i]) + 10 >= np.argmax(specs_y[i]) and np.argmax(specs_y[i]) - 10 <= np.argmax(specs_y[i]):
        correct += 1
